...
def trainScore_hydra_gpu( dataset , device, batch_size ):
    ...
    error_explain_set = model.score(data_explain)
...
def train_Minirocket_ridge_GPU(  dataset , device, batch_size ):
    ...
    acc_explain = model.score(data_explain)
    acc_test = model.score(data_test)

    return acc_explain.item(), acc_test.item(), model
...
def train_ConvTran( dataset , device, batch_size, verbose=False ):
    ...
    accuracy_explainSet =   convTran.score(explain_loader)
