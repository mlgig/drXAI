...
class Dataset(DataLoader):
    ...
    def __init__(self, X, Y, batch_size = 256, shuffle = True, **kwargs):
        ...
        self._mmap_X = X #np.load(path_X, mmap_mode = "r")
        self._mmap_Y = Y  #np.load(path_Y, mmap_mode = "r")
...
class BatchDataset(DataLoader):
    ...
    def _reset(self):
        ...
        if num_blocks > 1:
            step = int(max_size - int(surplus / (num_blocks)))
        else:
            step = max_size
        ...
        for i in range(num_blocks):
            ...
            a = i * step
            b = min(a + step, n)    #min(a + max_size, n)
